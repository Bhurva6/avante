    ...
    def _login(self) -> bool:
        ...
        try:
            ...
            if data.get('status') == 'success' and token:
                ...
                logger.info("✅ IOSPL ERP login successful")
                return True

            logger.error(f"❌ IOSPL ERP login failed: {data.get('message', 'Unknown error')}")
            return False
        except requests.exceptions.RequestException as e:
            logger.error(f"❌ IOSPL ERP login request failed: {str(e)}")
            return False
